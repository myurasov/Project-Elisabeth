...
function createCommand(pot = 0, tilt = 0, claw = 0, time = 400) {
  
  local t, p5 = 1500, p6 = 2430, p4 = 2350, p3 = 500, p1 = 1350;
  
  // base rotation
  p5 = round(-1000 * pot + 1500, 0);
  
  // claw
  p1 = 1350 - (1350 - 900) * claw;
  ...
  return format(
    "#5p%u #6p%u #4p%u #3p%u #2p1950 #1p%u t%d",
    p5, p6, p4, p3, p1, time);
}

function send(command) {
  server.log("send: " + command);
  pubNub.publish(CHANNEL, command);
}

// --

// init
send("#5p1500 #6p2430 #4p2350 #3p500 #2p1950 #1p1350 t2000");

pot <- 0;
tilt <- 0;
claw <- 1;

device.on("pot", function (val) {
  pot = val;
  send(createCommand(pot, tilt, claw));
});

device.on("tilt", function (val) {
  if (val < 1.8 && val > -1 /* ignore extreme values */) {
    tilt = val;
    send(createCommand(pot, tilt, claw));
  }
});

device.on("claw", function (val) {
  //claw = val;
  send(createCommand(pot, tilt, claw));
});